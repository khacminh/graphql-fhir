...
describe('Router Utils Test', () => {
  ...
  describe('configureRoutes', () => {
    ...
    test('should invoke the expressGraphql module with options and context', () => {
      ...
      const useMock = server.app.use.mock;
      const args = useMock.calls[mock.calls.length - 1];
    ...
    test('should add a graphiql endpoint if IS_PRODUCTION is false', () => {
      ...
      const args = mock.calls[mock.calls.length - 1];
    ...
    test('should skip over empty exported profiles', () => {
      ...
      const expressGraphqlArgs = mock.calls[0][0]();
      const rootSchema = expressGraphqlArgs.schema;
  ...
  describe('parseVersionFromUrl', () => {
    ...
    test('should parse the version from the provided url', () => {
      ...
      const version = parseVersionFromUrl('/3_0_1/graphiql', {
    ...
    test('should default to the defaultVersion if unable to parse a valid version', () => {
      const version = parseVersionFromUrl('/12_0_2/graphiql', {
        defaultVersion: '3_0_2',
      });
  ...
  describe('graphqlErrorFormatter', () => {
    test('should return an error formatting function', () => {
      const errorFormatter = graphqlErrorFormatter(logger, VERSION['3_0_1']);

      expect(typeof errorFormatter).toBe('function');
    });

    test('the error formatting function should return JSON formatted for GraphQL', () => {
      const mockOperationOutcome = { issue: [{ diagnostics: 'FUBAR' }] };
      const mockError = formatErrorForGraphQL(mockOperationOutcome);
      const errorFormatter = graphqlErrorFormatter(logger, VERSION['3_0_1']);
      const results = errorFormatter(mockError);

      // All four of these keys should be present, even if some of the values are undefined
      const propertyKeys = Object.keys(results);
      expect(propertyKeys.indexOf('path')).not.toBe(-1);
      expect(propertyKeys.indexOf('message')).not.toBe(-1);
      expect(propertyKeys.indexOf('locations')).not.toBe(-1);
      expect(propertyKeys.indexOf('extensions')).not.toBe(-1);
    });
    ...
    test('the error formatting function should create an operation outcome if extensions are not present', () => {
      const mockError = new GraphQLError('NormalError');
      const errorFormatter = graphqlErrorFormatter(logger, VERSION['3_0_1']);
      const results = errorFormatter(mockError);
