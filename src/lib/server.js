...
class Server {
  ...
  configureSession(session) {
    ...
    const { sessionStore } = this.config;
  ...
  setPublicDirectory(publicDir = '') {
    ...
    const { publicDirectory } = this.config;
  ...
  enableHealthCheck() {
    ...
    this.app.use('/healthcheck', (_req, res) => res.status(200).json({ uptime: process.uptime() }));
    // return self for chaining
    return this;
  }
  ...
  setErrorRoutes() {
    ...
    this.app.use((err, req, res, next) => {
      ...
      if (err) {
        const version = parseVersionFromUrl(req.path, this.config);
        const error = errorUtils.internal(version, err.message);
    ...
    this.app.use((req, res) => {
      ...
      const version = parseVersionFromUrl(req.path, this.config);
      const error = errorUtils.notFound(version);
      // Log the error and send the response
